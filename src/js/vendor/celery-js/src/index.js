...
(function(root, undefined) {
  ...
  Celery.prototype.initializeSlugs = function(slugs) {

    var self = this;

    var successCb = $.proxy(function(err, data) {
      var slug = data.data.product._id;
      self._setSlugData(slug, data);
    });
  ...
  Celery.prototype.fetchShop = function(slug, callback) {
    ...
    var successCb = $.proxy(function(err, data, has_slug_data) {
      if (data || data.data) {
        this._setUserId(data.data.user_id);

        if (!has_slug_data) {
          this._setSlugData(slug, data);
        }
      }
    ...
    if (this._hasSlugData(slug)) {
      return successCb(null, this._getSlugData(slug), true);
    }
  ...
  Celery.prototype._hasSlugData = function(slug) {
    return this.cached_slugs[slug] != undefined;
  }

  Celery.prototype._getSlugData = function(slug) {
    return this.cached_slugs[slug];
  }

  Celery.prototype._setSlugData = function(slug, data) {
    this.cached_slugs[slug] = data;
  }
